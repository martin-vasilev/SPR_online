...
for(i in 1:length(files)){ # for each participant file..
  ...
  be<-  t[which(t$sender== "Trap_instruction")[2:5]-1, ]
  ...
  if(t$which_list[1]== "A" | t$which_list[1]== "E"){
    t_b$sound<- c("Silence", "Lyrical", "Instrumental", "Speech")
  }
  
  if(t$which_list[1]== "B" | t$which_list[1]== "FALSE"){
    t_b$sound<- c("Lyrical", "Instrumental", "Speech", "Silence")
  }
  
  if(t$which_list[1]== "C" | t$which_list[1]== "G"){
    t_b$sound<- c("Instrumental", "Speech", "Silence", "Lyrical")
  }
  
  if(t$which_list[1]== "D" | t$which_list[1]== "H"){
    t_b$sound<- c("Speech", "Silence", "Lyrical", "Instrumental")
  }
...
write.csv(block, "Experiment3/data/block_info.csv", row.names = F) # save block info data
...
write.csv(dem, "Experiment3/data/participant_data.csv", row.names = F) # save device info


### DEVICE DATA:
device<- LabJs_device(dat) # extract devide info from lab.js
write.csv(device, "Experiment3/data/device_info.csv", row.names = F) # save device info
